...
namespace Cobalt {
	...
	Curl::Curl() {
		auto pCurlSetoptAddress = Memory::FindPattern
		(
		...
		auto pCurlEasySetoptAddress = Memory::FindPattern
		(
		...
		mpCurlEasySetoptHook = new Hook(reinterpret_cast<uintptr_t>(pCurlEasySetoptAddress), reinterpret_cast<uintptr_t>(CurlEasySetopt));
		if (!mpCurlEasySetoptHook->bSuccess) {
			printf("Initializing hook for CurlEasySetopt has failed, exiting immediately!\n");
			exit(EXIT_FAILURE);
		}
	}
	Curl::~Curl() {
		if (mpCurlEasySetoptHook) {
			delete mpCurlEasySetoptHook;
		}
	}
}